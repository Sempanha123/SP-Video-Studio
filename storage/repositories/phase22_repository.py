...
class Phase22Repository:
    ...
    def speaker(self, project_id: str, speaker_id: str) -> SpeakerProfile | None:
        with self.database.connect() as c:
            row = c.execute(
                "SELECT * FROM speakers WHERE id=? AND project_id=?", (speaker_id, project_id)
            ).fetchone()
    ...
    def blocks_for_section(self, section_id: str) -> list[SpeechBlock]:
        with self.database.connect() as c:
            rows = c.execute(
                "SELECT * FROM speech_blocks WHERE script_section_id=? ORDER BY block_order,id", (section_id,)
            ).fetchall()
    ...
    def block(self, project_id: str, block_id: str) -> SpeechBlock | None:
        ...
        return SpeechBlock.from_record(row) if row else None
    ...
    def save_block(self, project_id: str, item: SpeechBlock) -> SpeechBlock:
        ...
        with self.database.connect() as c, c:
            ...
            if owner is None:
                raise KeyError("Script section not found in this project.")
            c.execute(
                """INSERT INTO speech_blocks(id,script_section_id,block_order,speaker_id,text,language,voice_override_id,
                   speech_source_type,pause_before_ms,pause_after_ms,scene_id,start_offset_ms,audio_id,metadata_json,created_at,updated_at)
                   VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                   ON CONFLICT(id) DO UPDATE SET block_order=excluded.block_order,speaker_id=excluded.speaker_id,
                   text=excluded.text,language=excluded.language,voice_override_id=excluded.voice_override_id,
                   speech_source_type=excluded.speech_source_type,pause_before_ms=excluded.pause_before_ms,
                   pause_after_ms=excluded.pause_after_ms,scene_id=excluded.scene_id,start_offset_ms=excluded.start_offset_ms,
                   audio_id=excluded.audio_id,metadata_json=excluded.metadata_json,updated_at=excluded.updated_at""",
                (item.id,item.script_section_id,item.order,item.speaker_id or None,item.text,item.language,item.voice_override_id,
                 item.source_type_code,item.pause_before_ms,item.pause_after_ms,item.scene_id or None,item.start_offset_ms,
                 item.audio_id,_json(item.metadata),item.created_at,item.updated_at),
            )
        return item
    ...
    def delete_block(self, project_id: str, block_id: str) -> None:
        with self.database.connect() as c, c:
            ...
            if cur.rowcount == 0:
                raise KeyError("Speech block not found.")
    ...
    def normalize_block_order(self, project_id: str, section_id: str) -> list[SpeechBlock]:
        ...
        with self.database.connect() as c, c:
            for order, item in enumerate(items):
                c.execute(
                    """UPDATE speech_blocks SET block_order=?,updated_at=? WHERE id=? AND script_section_id=? AND
                       script_section_id IN (SELECT s.id FROM script_sections s JOIN scripts sc ON sc.id=s.script_id WHERE sc.project_id=?)""",
                    (order, utc_now_iso(), item.id, section_id, project_id),
                )
    ...
    def ensure_legacy_block(self, project_id: str, section_id: str, *, default_language: str = "en") -> list[SpeechBlock]:
        ...
        language = str(row["language"] or default_language)
        item = SpeechBlock(script_section_id=section_id, order=0, text=text, language=language, speech_source_type=("tts" if text.strip() else "none"))
        self.save_block(project_id, item)
        return [item]
    ...
    def save_state(self, project_id: str, **updates: object) -> dict[str, object]:
        ...
        current.update({k: v for k, v in updates.items() if k in {"previewQuality","activeSceneId","activeLayerId","activeSpeakerId","metadata"}})
        now = utc_now_iso()
        with self.database.connect() as c, c:
            c.execute(
                """INSERT INTO phase22_project_state(project_id,preview_quality,active_scene_id,active_layer_id,active_speaker_id,metadata_json,updated_at)
                   VALUES(?,?,?,?,?,?,?) ON CONFLICT(project_id) DO UPDATE SET preview_quality=excluded.preview_quality,
                   active_scene_id=excluded.active_scene_id,active_layer_id=excluded.active_layer_id,
                   active_speaker_id=excluded.active_speaker_id,metadata_json=excluded.metadata_json,updated_at=excluded.updated_at""",
                (project_id,str(current.get("previewQuality","auto")),str(current.get("activeSceneId","")),
                 str(current.get("activeLayerId","")),str(current.get("activeSpeakerId","")),
                 _json(current.get("metadata",{})),now),
            )
        return current

    def state(self, project_id: str) -> dict[str, object]:
        with self.database.connect() as c:
            row = c.execute("SELECT * FROM phase22_project_state WHERE project_id=?", (project_id,)).fetchone()
        if row is None:
            return {"previewQuality":"auto","activeSceneId":"","activeLayerId":"","activeSpeakerId":"","metadata":{}}
        try: meta = json.loads(str(row["metadata_json"] or "{}"))
        except Exception: meta = {}
        return {"previewQuality":str(row["preview_quality"]),"activeSceneId":str(row["active_scene_id"]),
                "activeLayerId":str(row["active_layer_id"]),"activeSpeakerId":str(row["active_speaker_id"]),
    ...
    def duplicate_project(self, source_project_id: str, target_project_id: str, *, media_map: dict[str,str] | None=None, scene_map: dict[str,str] | None=None) -> dict[str, dict[str, str]]:
        maps: dict[str, dict[str, str]] = {"speaker": {}, "block": {}, "audioClip": {}}
        target_sections = self._section_map(source_project_id, target_project_id); media_map=media_map or {}; scene_map=scene_map or {}
        for speaker in self.speakers(source_project_id):
            old = speaker.id; clone = deepcopy(speaker); clone.speaker_id = str(uuid4()); clone.project_id = target_project_id
            clone.created_at = clone.updated_at = utc_now_iso(); self.save_speaker(clone); maps["speaker"][old] = clone.id
        for block in self.blocks_for_project(source_project_id):
            target_section = target_sections.get(block.script_section_id)
            if not target_section:
                continue
            old = block.id; clone = deepcopy(block); clone.block_id = str(uuid4()); clone.script_section_id = target_section
            clone.speaker_id = maps["speaker"].get(block.speaker_id, "") if block.speaker_id else ""
            clone.scene_id = scene_map.get(block.scene_id, "") if block.scene_id else ""
            clone.audio_id = ""  # Generated block audio is regenerated so voice/text metadata cannot become stale.
        ...
        for clip in self.audio_clips(source_project_id):
            ...
            old=clip.id; clone=deepcopy(clip); clone.clip_id=str(uuid4()); clone.project_id=target_project_id; clone.media_id=mapped_media
            clone.scene_id=scene_map.get(clip.scene_id,"") if clip.scene_id else ""; clone.created_at=clone.updated_at=utc_now_iso(); self.save_audio_clip(clone); maps["audioClip"][old]=clone.id
    ...
    def _section_map(self, source_project_id: str, target_project_id: str) -> dict[str, str]:
        with self.database.connect() as c:
            source = c.execute(
                """SELECT s.id,s.section_order FROM script_sections s JOIN scripts sc ON sc.id=s.script_id
                   WHERE sc.project_id=? ORDER BY s.section_order""", (source_project_id,)
            ).fetchall()
            target = c.execute(
                """SELECT s.id,s.section_order FROM script_sections s JOIN scripts sc ON sc.id=s.script_id
                   WHERE sc.project_id=? ORDER BY s.section_order""", (target_project_id,)
            ).fetchall()
        by_order = {int(row["section_order"]): str(row["id"]) for row in target}
        return {str(row["id"]): by_order.get(int(row["section_order"]), "") for row in source}
