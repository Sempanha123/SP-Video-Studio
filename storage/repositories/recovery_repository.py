...
class RecoveryRepository:
    ...
    def get_autosave(self,project_id:str)->ProjectAutosaveState|None:
        with self.database.connect() as c:
            r=c.execute("SELECT * FROM autosave_state WHERE project_id=?",(project_id,)).fetchone()
    ...
    def save_autosave(self,s:ProjectAutosaveState)->None:
        with self.database.connect() as c,c:
            c.execute("""INSERT INTO autosave_state(project_id,project_revision,saved_revision,status,last_modified_at,last_saved_at,scheduled_at,saving_revision,dirty_topics_json,failure_message)
            VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(project_id) DO UPDATE SET project_revision=excluded.project_revision,saved_revision=excluded.saved_revision,status=excluded.status,last_modified_at=excluded.last_modified_at,last_saved_at=excluded.last_saved_at,scheduled_at=excluded.scheduled_at,saving_revision=excluded.saving_revision,dirty_topics_json=excluded.dirty_topics_json,failure_message=excluded.failure_message""",
            (s.project_id,s.project_revision,s.saved_revision,s.status_code,s.last_modified_at,s.last_saved_at,s.scheduled_at,s.saving_revision,_json(sorted(s.dirty_topics)),s.failure_message))
    ...
    def save_session(self,s:RecoverySession)->None:
        with self.database.connect() as c,c:
            c.execute("""INSERT INTO recovery_sessions(id,started_at,closed_at,clean_shutdown,app_version,metadata_json) VALUES(?,?,?,?,?,?)
            ON CONFLICT(id) DO UPDATE SET closed_at=excluded.closed_at,clean_shutdown=excluded.clean_shutdown,metadata_json=excluded.metadata_json""",(s.session_id,s.started_at,s.closed_at,int(s.clean_shutdown),s.app_version,_json(s.metadata)))
    def save_snapshot(self,s:RecoverySnapshot)->None:
        with self.database.connect() as c,c:
            c.execute("""INSERT INTO recovery_snapshots(id,project_id,session_id,project_revision,created_at,reason,snapshot_type,status,path,size,checksum,metadata_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)
            ON CONFLICT(id) DO UPDATE SET status=excluded.status,path=excluded.path,size=excluded.size,checksum=excluded.checksum,metadata_json=excluded.metadata_json""",(s.id,s.project_id,s.session_id,s.project_revision,s.created_at,s.reason,s.type_code,s.status_code,s.path,s.size,s.checksum,_json(s.metadata)))
    ...
    def snapshots(self,project_id:str|None=None)->list[RecoverySnapshot]:
        with self.database.connect() as c:
            rows=c.execute("SELECT * FROM recovery_snapshots"+(" WHERE project_id=?" if project_id else "")+" ORDER BY created_at DESC",((project_id,) if project_id else ())).fetchall()
    ...
    def upsert_interrupted(self,job_key,project_id,job_type,source_id,state,retry_mode,detected_at,metadata):
        with self.database.connect() as c,c:c.execute("""INSERT INTO interrupted_jobs(job_key,project_id,job_type,source_id,state,retry_mode,detected_at,metadata_json) VALUES(?,?,?,?,?,?,?,?)
        ON CONFLICT(job_key) DO UPDATE SET state=excluded.state,retry_mode=excluded.retry_mode,detected_at=excluded.detected_at,metadata_json=excluded.metadata_json""",(job_key,project_id,job_type,source_id,state,retry_mode,detected_at,_json(metadata)))
