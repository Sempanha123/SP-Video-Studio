...
class TimelineTrackType(StrEnum):
    OVERLAY="overlay"; VIDEO="video"; BROLL="broll"; SUBTITLE="subtitle"; VOICE="voice"; SOURCE_AUDIO="source_audio"; MUSIC="music"; SFX="sfx"

DEFAULT_TRACKS: tuple[tuple[str,str],...]=(
    (TimelineTrackType.OVERLAY.value,"V3 Presenter / Overlay"),
    (TimelineTrackType.BROLL.value,"V2 B-roll / Graphics"),
    (TimelineTrackType.VIDEO.value,"V1 Main Video"),
    (TimelineTrackType.SUBTITLE.value,"Subtitles"),(TimelineTrackType.VOICE.value,"Voice"),
    (TimelineTrackType.SOURCE_AUDIO.value,"Source Audio"),(TimelineTrackType.MUSIC.value,"Music / Manual Audio"),(TimelineTrackType.SFX.value,"SFX"),
)

@dataclass(slots=True)
class TimelineTrack:
    project_id:str; track_type:str|TimelineTrackType; name:str; order:int; visible:bool=True; muted:bool=False; locked:bool=False; height:int=64; track_id:str=field(default_factory=lambda:str(uuid4())); metadata:dict[str,Any]=field(default_factory=dict)
    @property
    ...
    def to_dict(self)->dict[str,Any]:
        self.validate();return {"id":self.id,"projectId":self.project_id,"type":self.type_code,"name":self.name,"order":self.order,"visible":self.visible,"muted":self.muted,"locked":self.locked,"height":self.height,"metadata":dict(self.metadata)}
    @classmethod
