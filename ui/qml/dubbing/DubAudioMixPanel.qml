...
AppCard {
    ...
    ColumnLayout { anchors.fill:parent; anchors.margins:Theme.spacing.lg; spacing:Theme.spacing.sm
        ...
        RowLayout { Layout.fillWidth:true
            ...
            AppComboBox { id:mode; accessibleName:"Dub audio mix mode"; model:["replace","mix","duck","custom"]; currentIndex:Math.max(0,model.indexOf(controller.mixSettings.mode||"duck")) }
        ...
        RowLayout { Layout.fillWidth:true
            ...
            Slider { id:original; from:0; to:1.5; value:Number(controller.mixSettings.originalVolume||0.25); Layout.fillWidth:true; Accessible.name:"Original audio volume, "+Math.round(value*100)+" percent"; Accessible.role:Accessible.Slider }
            Text { text:Math.round(original.value*100)+"%"; color:Theme.colors.textMuted }
            Text { text:"Dub"; color:Theme.colors.textSecondary }
            Slider { id:dub; from:0; to:1.5; value:Number(controller.mixSettings.dubVolume||1); Layout.fillWidth:true; Accessible.name:"Dub audio volume, "+Math.round(value*100)+" percent"; Accessible.role:Accessible.Slider }
