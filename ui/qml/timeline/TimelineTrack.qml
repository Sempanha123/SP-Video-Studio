...
Item {
    ...
    property var visibleClips: {
        var clips=track.clips||[]; if(visibleEndMs<=visibleStartMs)return clips
        var start=Math.max(0,visibleStartMs-preloadMarginMs),end=visibleEndMs+preloadMarginMs,result=[]
        for(var i=0;i<clips.length;++i){var clip=clips[i],clipStart=Number(clip.startMs||0),clipEnd=Number(clip.endMs||(clipStart+Number(clip.durationMs||0)));if(clipEnd>=start&&clipStart<=end)result.push(clip)}
        return result
    }
    width:timelineWidth;height:Number(track.height||64)
    ...
    function accent(kind){if(kind==="video")return Theme.colors.video;if(kind==="overlay")return Theme.colors.story;if(kind==="subtitle")return Theme.colors.translate;if(kind==="voice")return Theme.colors.info;if(kind==="source_audio")return Theme.colors.success;return Theme.colors.surfacePressed}
