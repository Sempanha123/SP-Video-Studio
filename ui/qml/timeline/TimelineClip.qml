...
Rectangle {
    ...
    signal activated(string clipId)
    ...
    Text {
        anchors.left: parent.left; anchors.leftMargin: 8; anchors.right: parent.right; anchors.rightMargin: 8; anchors.verticalCenter: parent.verticalCenter
        text: root.width > 70 ? (clipData.label || clipData.sourceType || "Clip") : ""
        color: Theme.colors.textPrimary; elide: Text.ElideRight; font.family: Theme.type.family; font.pixelSize: Theme.type.caption
    }
    Text { anchors.right: parent.right; anchors.rightMargin: 6; anchors.bottom: parent.bottom; anchors.bottomMargin: 3; visible: root.width > 110; text: ((Number(clipData.durationMs || 0)/1000).toFixed(1)) + "s"; color: Theme.colors.textMuted; font.family: Theme.type.family; font.pixelSize: 9 }
    Text { anchors.left: parent.left; anchors.leftMargin: 5; anchors.top: parent.top; anchors.topMargin: 2; visible: clipData.metadata && clipData.metadata.missing; text: "Missing"; color: Theme.colors.warning; font.family: Theme.type.family; font.pixelSize: 9 }
    Text { anchors.right: parent.right; anchors.rightMargin: 5; anchors.top: parent.top; anchors.topMargin: 2; visible: clipData.metadata && clipData.metadata.transitionType && clipData.metadata.transitionType !== "cut"; text: String(clipData.metadata.transitionType) + " " + String(clipData.metadata.transitionDurationMs || 0) + "ms"; color: Theme.colors.info; font.family: Theme.type.family; font.pixelSize: 9 }
    ...
    MouseArea {
        ...
        onPressed: { root.activated(String(root.clipData.id || "")); if (mouse.button === Qt.RightButton) { var p=mapToItem(null,mouse.x,mouse.y); root.contextRequested(root.clipData,p.x,p.y) } }
