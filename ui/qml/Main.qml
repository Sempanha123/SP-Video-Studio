...
ApplicationWindow {
    ...
    function navigate(page,context){ currentPage=page; if(page==="settings"){if(context)settingsSection=context}else if(context)selectedWorkflow=context }
    function readinessState(){return typeof readinessController!=="undefined"?readinessController.readiness:({})}
    Shortcut { sequence:"Ctrl+S"; onActivated: Recovery.manualSave() }
            ...
            ColumnLayout { Layout.fillWidth:true;Layout.fillHeight:true;spacing:0
                ...
                Item {Layout.fillWidth:true;Layout.fillHeight:true
                    Loader {id:pageLoader;anchors.fill:parent;anchors.margins:window.currentPage==="workspace"?Theme.spacing.md:Theme.spacing.xlg;source:window.pageSource(window.currentPage);opacity:status===Loader.Ready?1:0
                        onLoaded:{if(window.currentPage==="create")item.selectedWorkflow=window.selectedWorkflow;if(window.currentPage==="settings")item.section=window.settingsSection}
    ...
    Component.onCompleted:{if(typeof settingsController!=="undefined")Theme.setMode(settingsController.theme); if(typeof projectController!=="undefined")Recovery.setCurrentProject(projectController.currentProject.id||"")}
