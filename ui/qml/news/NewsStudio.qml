...
Item {
    ...
    function stepForTab(){var m={overview:0,sources:0,claims:1,brief:2,script:2,visuals:4};return m[currentTab]===undefined?0:m[currentTab]}
    ...
    ColumnLayout{anchors.fill:parent;spacing:Theme.spacing.md
        PageHeader{Layout.fillWidth:true;title:"News Studio";description:"Editorial workflow with evidence, review and clear publishing readiness."}
        WorkflowStepper{Layout.fillWidth:true;steps:["Sources","Claims","Script","Voice","Visuals","Export"];currentIndex:root.stepForTab();onStepRequested:function(i){if(i===0)root.currentTab="sources";else if(i===1)root.currentTab="claims";else if(i===2)root.currentTab="script";else if(i===3)root.navigateRequested("voices");else if(i===4)root.currentTab="visuals";else root.navigateRequested("export")}}
        SegmentTabs{items:["Overview","Sources","Claims","Brief","Script","Visuals"];currentIndex:["overview","sources","claims","brief","script","visuals"].indexOf(root.currentTab);onActivated:function(i){root.currentTab=["overview","sources","claims","brief","script","visuals"][i]}}
