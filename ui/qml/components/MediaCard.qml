...
AppCard {
    ...
    selected: isSelected
    implicitHeight: 226
    onClicked: root.activated(root.mediaId)

    function secondaryLine() {
        var values = []
        if (root.resolution !== "") values.push(root.resolution)
        if (root.duration !== "") values.push(root.duration)
        if (values.length === 0) values.push(root.typeName)
        return values.join(" · ")
    }
        ...
        Rectangle {
            ...
            Column {
                ...
                Icon {
                    ...
                    width: 30
                    height: 30
                    name: root.mediaType === "audio" ? "audio" : (root.mediaType === "image" ? "image" : "video")
                }
                Text {
                    anchors.horizontalCenter: parent.horizontalCenter
                    text: root.mediaType === "audio" ? "Audio" : (root.mediaStatus === "missing" ? "File Missing" : root.typeName)
                    color: root.mediaStatus === "missing" ? Theme.colors.warning : Theme.colors.textMuted
