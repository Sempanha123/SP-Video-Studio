...
Item { id: root; property var controller; signal itemSelected(var itemData)
    ...
    ColumnLayout { anchors.fill: parent; spacing: Theme.spacing.sm
        RowLayout { Layout.fillWidth: true; spacing: Theme.spacing.sm
            AppComboBox { id: filter; Layout.preferredWidth: 136; model: ["all","running","pending","completed","failed","skipped"]; onActivated: { searchDebounce.stop(); controller.setQueueFilter(currentText, search.text) } }
        ...
        ListView { id: queueView; Layout.fillWidth: true; Layout.fillHeight: true; clip: true; spacing: 2; reuseItems: true; cacheBuffer: 420; model: controller ? controller.items : []
